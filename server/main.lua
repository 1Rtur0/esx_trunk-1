...
RegisterServerEvent('esx_trunk:getOwnedVehicles')
AddEventHandler('esx_trunk:getOwnedVehicles', function()
	local vehicles = {}
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	MySQL.Async.fetchAll(
	'SELECT * FROM owned_vehicles WHERE owner = @owner', { ['@owner'] = xPlayer.identifier },
	function(result)
		...
		TriggerClientEvent('esx_trunk:setOwnedVehicle', _source, vehicles)
	end)
end)
...
RegisterServerEvent('esx_trunk:getInventory')
AddEventHandler('esx_trunk:getInventory', function(plate)
	local inventory_ = {}
	local _source = source
	MySQL.Async.fetchAll(
		'SELECT * FROM `truck_inventory` WHERE `plate` = @plate', { ['@plate'] = plate },
		function(inventory)
		...
		local xPlayer = ESX.GetPlayerFromId(_source)
		TriggerClientEvent('esx_trunk:getInventoryLoaded', xPlayer.source, inventory_,weight)
	end)
end)


RegisterServerEvent('esx_trunk:removeInventoryItem')
AddEventHandler('esx_trunk:removeInventoryItem', function(plate, item, itemType, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_trunk:addInventoryItem')
AddEventHandler('esx_trunk:addInventoryItem', function(type, model, plate, item, qtty, name,itemType, ownedV)
	local _source = source
	local xPlayer	= ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_trunk:AddVehicleList')
AddEventHandler('esx_trunk:AddVehicleList', function(plate)
...
RegisterServerEvent('esx_trunk:RemoveVehicleList')
AddEventHandler('esx_trunk:RemoveVehicleList', function(plate)
