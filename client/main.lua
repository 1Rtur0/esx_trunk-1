...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerEvent("esx_trunk:getOwnedVehicles")
end)
...
RegisterNetEvent('esx_trunk:setOwnedVehicle')
AddEventHandler('esx_trunk:setOwnedVehicle', function(vehicle)
...
function OpenMenuVehicle()
	...
	if globalPlate ~= nil or globalPlate ~= "" or globalPlate ~= " " then
		ESX.TriggerServerCallback('esx_truck:checkvehicle',function(isBusy)
			if not isBusy then
				...
				if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
					...
					if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
						...
						if locked == 1 or class == 15 or class == 16 or class == 14 then
							...
							if globalPlate ~= nil or globalPlate ~= "" or globalPlate ~= " " then
								vehicleNearby = true
								TriggerServerEvent('esx_trunk:AddVehicleList', globalPlate)
								TriggerServerEvent("esx_trunk:getInventory", GetVehicleNumberPlateText(vehFront))
							end
						else
							ESX.ShowNotification(_U('bagage_locked'))
						end
					end
				else
					ESX.ShowNotification(_U('no_vehicle_proximity'))
				end
				lastOpen = true
				GUI.Time = GetGameTimer()
			else
				TriggerEvent('esx:showNotification', _U('bagage_busy'))
			end
		end, globalPlate)
	end
end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys["I"]) and (GetGameTimer() - GUI.Time) > 1000 then
			if count == 0 then
				OpenMenuVehicle()
				count = count + 1
			else
		...
		elseif lastOpen and IsControlPressed(0, Keys["BACKSPACE"]) and (GetGameTimer() - GUI.Time) > 150 then
			...
			if lastVehicle > 0 then
				...
				TriggerServerEvent('esx_trunk:RemoveVehicleList', lastvehicleplatetext)
				lastVehicle = 0
			end
			GUI.Time = GetGameTimer()
		end
	...
	while true do
		...
		if vehicleNearby then
			...
			if DoesEntityExist(vehicle) then
				...
				TriggerServerEvent('esx_trunk:RemoveVehicleList', globalPlate)
				vehicleNearby = false
				lastOpen = false
				ESX.UI.Menu.CloseAll()
				SetVehicleDoorShut(lastVehicle, 5, false)
...
RegisterNetEvent('esx_trunk:getInventoryLoaded')
AddEventHandler('esx_trunk:getInventoryLoaded', function(inventory,weight)
	local elements = {}
		...
		for i=1, #weaponList, 1 do
			...
			if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0	then
				...
				if not max then
					local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
					local closecar = GetClosestVehicle(x, y, z, 4.0, 0, 71)
					TriggerServerEvent('esx_trunk:addInventoryItem', GetVehicleClass(closecar), GetDisplayNameFromVehicleModel(GetEntityModel(closecar)), GetVehicleNumberPlateText(vehFront), data3.current.value, quantity, data3.current.name, data3.current.type, ownedV)
				ESX.ShowNotification(_U('bagage_weight', Kgweight, MaxVh))
				Citizen.Wait(500)
				TriggerServerEvent("esx_trunk:getInventory", GetVehicleNumberPlateText(vehFront))
				else
				ESX.ShowNotification(_U('reached_limit', MaxVh))
				end
			else
			...
			if quantity > 0 and quantity <= tonumber(data.current.count) and vehFront > 0 then
				if not max then
					TriggerServerEvent('esx_trunk:removeInventoryItem', GetVehicleNumberPlateText(vehFront), data.current.value, data.current.type, quantity)
					local typeVeh = GetVehicleClass(vehFront)
					local MaxVh =(tonumber(Config.VehicleLimit[typeVeh])/1000)
					local Itemweight =tonumber(getItemyWeight(data.current.value)) * quantity
					local totalweight = tonumber(weight) - Itemweight
